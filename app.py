...
status_box.caption("Building clinical development, enrollment, modality, catalyst, and evidence views…")
...
# 2. Status comes first because it tells him what is alive vs inactive.
...
# 3. Core competitive ADC views near the top.
...
section("Clinical Trial Timeline")
st.plotly_chart(charts.timeline_chart(df), width="stretch", config=charts.CHART_CONFIG)

# 4. Forward-looking intelligence before detailed drilldowns.
...
# 5. Scale of patient populations and enrollment.
...
with st.expander("Enrollment detail: lane, phase, status, and ADC relevance", expanded=True):
    render_dark_table(lane_enroll_detail, height=360)
with st.expander("Enrollment detail: patient population, line of therapy, and prior-treatment context", expanded=True):
    render_dark_table(pop_enroll_detail, height=420)

# 6. Modality and ADC relevance.
...
with st.expander("Status × modality detail by lane, phase, sponsors, enrollment, and sites", expanded=True):
    render_dark_table(status_modality_detail, height=420)

# 7. Biology/therapy intelligence after the key operational views.
...
# 9. Phase/geography after higher-priority strategic views.
